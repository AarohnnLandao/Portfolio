...
<?php
    ...
    $uname = filter_input(INPUT_POST, 'first_name');
    /*echo "Fields: " . $email_address . $uname;*/
    
    // Validate inputs
    if ($email_address == null || $uname == null) {
        ...
        exit();
            ...
            $query = 'INSERT INTO newslettersignup
                         (visitorName, visitorEmail, modNumber)
                      VALUES
                         (:visitorName, :visitorEmail, 1)';
            $statement = $db->prepare($query);
            $statement->bindValue(':visitorName', $uname);
            $statement->bindValue(':visitorEmail', $email_address);            
            $statement->execute();
